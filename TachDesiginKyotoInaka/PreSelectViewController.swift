...
//
//  Created by 山口 智生 on 2015/09/19.
...
class PreSelectViewController: PagingViewController, PageControlDelegate {
    ...
    func moveTargetPage(targetPageTmp: Int) {
        ...
        if targetPage != self.getCurrentPageIndex() {
            ...
            self.pageViewController?.setViewControllers(viewControllers as? [UIViewController], direction: direction, animated: true, completion: {(Bool) in self.updateTab()})
        }
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        for indexes in (controller?.selectedItemIndexes)! {
            let tmpDatas: NSMutableArray = NSMutableArray()
            //tmpDatas = indexes.map{ return tmpDataList[$0] }
            
            for i in indexes {
                tmpDatas.addObject((self.pageData[index] as! [AnyObject])[i])
            }
            
            if tmpDatas.count > 0 {
                //型判定
                if tmpDatas[0] is Playlist {
                    manager.appendPlaylists(tmpDatas as AnyObject as! [Playlist])
                } else {
                    manager.appendSongs(tmpDatas as AnyObject as! [Song])
                }
            }
            index++
        }
